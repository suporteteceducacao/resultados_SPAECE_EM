...
2. Visualize e analise as tabela de resultadose variações percentuais.
3. Explore os gráficos gerados.
4. Faça o download dos gráficos e tabelas.

Obs.1: Para visualisar resultados por MUNICÍPIOS, basta selecionar o nome do municipio dentro do campo ESCOLA.
                 
Obs.2: Algumas escolas podem ter o nome alterado por razões técnicas, logo para um melhor resultado de busca dos dados utilize o campo INEP.
""")

# Título principal e subtítulo
st.title("📊 Dashboard de Análise de Desempenho por Escola e Município / 3º Série EM - SPAECE (2012 - 2023)")
...
with col3:
    etapa = st.selectbox('Selecione a Etapa', ['3º Série EM'])

# Adicionar campo para digitar o INEP da escola
inep_escola = st.text_input("Ou digite o INEP da escola:")

## Filtrar os dados com base nas seleções
if etapa in ['3º Série EM']:
        if inep_escola:
            filtered_data = result_spaece[(result_spaece['INEP_ESC'] == inep_escola)]
        else:
...
if filtered_data.empty:
    ...
    def criar_grafico_empilhado_unificado(tabela, etapa, componente_curricular, escola):
        ...
        ax.set_title(f'Gráfico Percentual por Padrão de Desempenho - {componente_curricular} - {escola} ({etapa})')
...
st.write("### Tabela de Variação Percentual por Edição")
...
st.markdown(
    """
    <p style="text-align: center; xmlns:cc="http://creativecommons.org/ns#" xmlns:dct="http://purl.org/dc/terms/">
      <span property="dct:title"><b>Dashboard de Análise de Resultados SPAECE Ensino Médio</b></span> &copy; 2025 is licensed under 
      <a href="https://creativecommons.org/licenses/by-nc-nd/4.0/?ref=chooser-v1" target="_blank" rel="noopener noreferrer" style="display:inline-block;">
        CC BY-NC-ND 4.0 </p>
    """,
    unsafe_allow_html=True,
)
